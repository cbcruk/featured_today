  ...
  async loader() {
    const row = await db
      .select({
        ...
        stories: sql<string>`GROUP_CONCAT(stories.story_id)`.mapWith(z.array(z.string()))
      })
      .from(schemas.countries)
      .leftJoin(schemas.stories, eq(schemas.countries.id, schemas.stories.countryId))
  ...
  async loader() {
    const rows = await db
      .select({
        ...
        apps: sql<string>`GROUP_CONCAT(apps.app_id)`.mapWith(z.array(z.string()))
      })
      .from(schemas.stories)
      .leftJoin(schemas.stories, eq(schemas.stories.id, schemas.artworks.storyId))
  ...
  async loader() {
    const rows = await db
      .select({
        ...
        categories: sql<string>`GROUP_CONCAT(appCategories.id)`.mapWith(z.array(z.number()))
      })
      .from(schemas.apps)
      .leftJoin(schemas.appOffers, eq(schemas.appOffers.appId, schemas.apps.id))
      .leftJoin(schemas.appCategories, eq(schemas.appCategories.appId, schemas.apps.id))
